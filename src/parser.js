...
const parser = (data) => {
  try {
    ...
    if (!feedTitle) {
      throw new Error('Не найден заголовок канала');
    }
    ...
    if (feedDescriptionBlock) {
      feedDescription = feedDescriptionBlock.innerHTML || feedDescriptionBlock.textContent;
    }
    ...
    const parsedItems = Array.from(items).map((item) => {
      ...
      let title = titleElement?.textContent || titleElement?.innerHTML || '';
      let link = linkElement?.textContent || linkElement?.innerHTML || '';
      let description = descriptionElement?.textContent || descriptionElement?.innerHTML || '';
